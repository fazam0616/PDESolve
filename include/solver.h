...
typedef struct {
    ...
    GridMetadata *grid_context; // Grid for spatial discretization (NULL for scalar systems)
...
// Set grid context for spatial discretization (NULL for scalar systems)
// sys: System to configure (must not be NULL)
// grid: Grid metadata (reference retained, caller owns)
// Memory: Stores pointer, does not take ownership
...
// Newton-Krylov iteration (Jacobian-free Newton with GMRES)
// sys: System to solve (must be well-posed)
// initial_guess: Starting values (can be NULL for zeros)
// Returns: SolverResult with solution (caller owns, must call solver_result_free)
// Algorithm: Jacobian-free Newton using finite differences J*v ≈ (F(x+εv) - F(x))/ε
// Inner solver: GMRES for linear system J*delta = -F
// Memory: Returns newly allocated SolverResult, caller must free
