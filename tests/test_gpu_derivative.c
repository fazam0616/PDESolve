...
static GridField* make_test_field(GridMetadata *grid) {
    ...
    for (uint32_t j = 0; j < grid->dims[1]; ++j) {
        for (uint32_t i = 0; i < grid->dims[0]; ++i) {
            ...
            Literal *lit = literal_create_scalar(sin(x) + cos(y));
...
int main(void) {
    ...
    GridField *cpu_dx = grid_field_derivative(u, 0, 1);
    Expression *expr_dx = expr_derivative(expr_variable("u"), "x");

    GPUProgram *prog = gpu_compile_optimized(expr_dx, grid, GPU_BACKEND_OPENGL);
    if (!prog) { fprintf(stderr, "SKIP: GPU compile not available for derivative\n"); grid_field_free(u); dict_free(dict); grid_metadata_free(grid); expression_free(expr_dx); return 0; }
    GridField *gpu_out = gpu_run_program_cpu(prog, dict, grid);
    if (!gpu_out) { fprintf(stderr, "SKIP: GPU derivative execution returned NULL\n"); gpu_program_free(prog); grid_field_free(u); dict_free(dict); grid_metadata_free(grid); expression_free(expr_dx); return 0; }

    double err = compute_relative_error_interior(cpu_dx, gpu_out);
    printf("GPU derivative relative error (interior check): %g\n", err);
    if (err >= 1e-4) { fprintf(stderr, "GPU derivative error exceeds tolerance\n"); return 2; }

    grid_field_free(u); grid_field_free(cpu_dx); grid_field_free(gpu_out);
    gpu_program_free(prog); dict_free(dict); grid_metadata_free(grid); expression_free(expr_dx);
    printf("test_gpu_derivative: PASS\n");
    return 0;
}
